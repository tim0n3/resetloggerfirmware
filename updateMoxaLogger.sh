...
# Change this to the path of your local Git repository
DATA_PATH="$(find "$HOME" -type d -name "pubsub" -exec find {} -type d -name "data" \;)"
# Change this to the path of your local logs folder
LOGS_PATH="$(find "$HOME" -type d -name "pubsub" -exec find {} -type d -name "logs" \;)"
...
create_log(){
	...
	check_error "create_log function experienced a critical failure. Please ensure you have sudo permissions and re-run the script."
...
backup_repo() {
    log "Creating backup..."
    tar --exclude="/home/pi/pubsub/data" \
        --exclude="/home/pi/pubsub/logs" \
